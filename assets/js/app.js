...
/**
 * 요소가 있을 때만 이벤트 핸들러 등록 (페이지 분리 후 누락 요소 대응)
 */
...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  // URL에 스냅샷이 있으면 스냅샷 뷰어 모드로 진입
  const snap = await loadSnapshotFromUrl();
  if (snap) {
    snapshotMode = snap;
    initSnapshotViewer(snap);
    return;
  }
  ...
  // 페이지 종류에 따른 분기:
  //  - body.page-main: 메인 페이지(파티 편성)
  //  - body.page-manage: 관리 페이지(API/원정대/레이드)
  //  manage.js가 별도로 관리 페이지 이벤트를 다루므로 여기서는 메인만 처리
  if (document.body.classList.contains('page-main')) {
    initMainPageEventListeners();
    renderRaids();
  }
...
/**
 * 스냅샷 뷰어 모드: 페이지 전체를 읽기 전용 스냅샷 표시로 대체
 */
function initSnapshotViewer(snap) {
  ...
  banner.innerHTML = `
    <div class="snapshot-banner-inner">
      <div>
        ...
        <span class="snapshot-timestamp">${escapeHtml(ts)}에 생성된 파티 편성 스냅샷</span>
      </div>
      <div>
        <button id="snapshot-exit-btn" class="btn-sm">내 편성 화면으로 이동</button>
      </div>
    </div>
  `;
  document.body.insertBefore(banner, document.querySelector('.site-header').nextSibling);

  // 나가기 버튼: ?s 파라미터 없는 기본 URL로 이동
  document.getElementById('snapshot-exit-btn').addEventListener('click', () => {
...
/**
 * 스냅샷 데이터를 메인 영역에 렌더링 (읽기 전용)
 */
function renderSnapshot(snap) {
  const container = $('#raids-list');
  if (!container) return;

  if (!snap.raids || snap.raids.length === 0) {
    container.innerHTML = '<p class="empty-msg">스냅샷에 포함된 파티가 없습니다.</p>';
    return;
  }

  // 레이드 하나당 섹션
  container.innerHTML = snap.raids.map(r => {
    const groupsHtml = r.groups.map((group, gi) => {
      const partiesHtml = group.parties.map((party, pi) => {
        const membersHtml = party.members.map(m => renderSnapshotChip(m)).join('');
        const warningHtml = party.hasWarning ? '<div class="party-warning">⚠ 서포터 부족</div>' : '';
        const incompleteHtml = party.isIncomplete ? `<div class="party-warning">⚠ 인원 미달 (${party.members.length}/4)</div>` : '';
        const partyLabel = escapeHtml(party.label || `파티 ${pi + 1}`);
        return `
          <div class="party-card ${party.hasWarning ? 'no-support' : ''} ${party.isIncomplete ? 'incomplete' : ''} ${group.isManual ? 'manual' : ''}">
            <div class="party-header">
              <span class="party-label snapshot-static-label">${partyLabel}</span>
              <span class="party-avg">
                <span class="party-avg-power">${escapeHtml(party.avgPower)}</span>
                <span class="party-avg-level">${escapeHtml(party.avgLevel)}</span>
              </span>
            </div>
            ${warningHtml}
            ${incompleteHtml}
      ...
      const groupLabel = escapeHtml(group.label || `공격대 ${gi + 1}`);
      const showHeader = group.parties.length > 1 || group.isManual || group.label;

      return `
        <div class="raid-group ${group.isManual ? 'is-manual' : ''}">
          ${showHeader ? `<div class="raid-group-header"><span class="snapshot-static-label">${groupLabel}</span></div>` : ''}
    ...
    // 남은 캐릭
    const leftoverHtml = (r.leftover && r.leftover.length > 0) ? `
      <div class="leftover-section">
        <h4 class="leftover-title">남은 캐릭터 <span class="hint">(공팟용)</span></h4>
        <div class="party-members leftover-zone">
          ${r.leftover.map(m => renderSnapshotChip(m)).join('')}
        </div>
      </div>
    ` : '';
      ...
      <article class="raid-card">
        <header class="raid-header">
          <div class="raid-title-group">
            <h3 class="raid-title">${escapeHtml(r.raidName)}</h3>
            <span class="raid-range">${escapeHtml(r.raidRange)}</span>
          </div>
        </header>
        ${groupsHtml}
        ${leftoverHtml}
...
/**
 * 스냅샷 칩 렌더링 (드래그 불가, 가벼운 버전)
 */
function renderSnapshotChip(char) {
  ...
  return `
    <div class="char-chip ${supportClass} snapshot-chip" title="[${escapeHtml(char.rosterRep)}] 원정대">
      <div class="chip-class">${escapeHtml(char.className)}</div>
      <div class="chip-name">${escapeHtml(char.name)}</div>
      <div class="chip-stats">
        <span class="chip-power">⚔ ${powerDisplay}</span>
...
function initMainPageEventListeners() {
  // 페이지에 없는 요소 참조 시 null이 되니, 사용 전 안전 체크.
  // $() 결과가 null이어도 메서드 호출이 무시되도록 wrapper 사용.
  ...
  // ═══ 스냅샷 공유 버튼 ═══
  const snapBtn = $('#snapshot-share-btn');
  if (snapBtn) {
    snapBtn.addEventListener('click', async () => {
      if (state.rosters.length === 0) {
        alert('먼저 원정대를 등록하고 파티를 편성해주세요.');
        return;
      }
      const raidCards = document.querySelectorAll('.raid-card');
      if (raidCards.length === 0) {
        alert('편성된 파티가 없습니다. 왼쪽에서 레이드를 선택해주세요.');
        return;
      }

      snapBtn.disabled = true;
      const originalText = snapBtn.textContent;
      snapBtn.textContent = '스냅샷 생성 중...';

      try {
        const url = await buildSnapshotUrl();

        // URL 길이 체크 (대부분의 환경에서 2KB 이상은 메시지 앱에서 잘릴 수 있음)
        ...
        if (copied) {
          showToast(`📸 스냅샷 링크가 복사되었습니다 (${(urlLen / 1024).toFixed(1)}KB)`);
        } else {
        ...
        if (urlLen > 4000) {
          setTimeout(() => {
            alert(`⚠ 스냅샷 URL이 ${(urlLen/1024).toFixed(1)}KB로 길어졌습니다.\n일부 메시지 앱(카카오톡 등)에서 링크가 잘릴 수 있으니,\n짧은 링크가 필요하면 bitly.com 같은 URL 단축 서비스를 사용하세요.`);
          }, 300);
        }
      } catch (e) {
        alert('스냅샷 생성 실패: ' + e.message);
      } finally {
      ...
      return `
        <tr class="${excluded ? 'excluded' : ''}">
          ...
          <td class="action-cell">
            ...
            <button class="char-delete-btn" data-delete-char="${c.id}" title="이 캐릭을 목록에서 완전히 삭제합니다. 원정대 갱신해도 다시 나타나지 않습니다.">×</button>
          </td>
        </tr>`;
    }).join('');
...
function renderRaidSelector() {
  ...
  // 각 카테고리 내부: minLevel 내림차순 (상위 레이드 먼저)
  for (const cat in byCategory) {
    byCategory[cat].sort((a, b) => b.minLevel - a.minLevel);
  }
  // 카테고리 자체도 최상위 레이드 기준 내림차순
  const sortedEntries = Object.entries(byCategory).sort(([, a], [, b]) => {
    return Math.max(...b.map(x => x.minLevel)) - Math.max(...a.map(x => x.minLevel));
  });
...
function renderRaids() {
  ...
  // 카테고리별 그룹핑 + 각 카테고리 내부 레이드는 minLevel 내림차순(상위 레이드 먼저)
  const byCategory = {};
  ...
  // 각 카테고리 내부 정렬: 상위 레벨(minLevel 높은 것)이 먼저
  for (const cat in byCategory) {
    byCategory[cat].sort((a, b) => b.raid.minLevel - a.raid.minLevel);
  }

  // 카테고리 자체도 "가장 상위 레벨 레이드를 보유한 카테고리"가 먼저 나오도록 정렬
  const sortedCategories = Object.entries(byCategory).sort(([, itemsA], [, itemsB]) => {
    const maxA = Math.max(...itemsA.map(x => x.raid.minLevel));
    const maxB = Math.max(...itemsB.map(x => x.raid.minLevel));
    return maxB - maxA;
  });

  container.innerHTML = sortedCategories.map(([cat, items]) => `
...
function renderRaidCard(raid, result) {
  ...
  // 자동 공격대 렌더링
  // 4인 레이드(공격대당 파티 1개)는 모든 파티를 하나의 grid에 평탄화하여 가로 배치 효과
  // 8인 레이드는 공격대별로 분리해서 헤더 표시
  let autoGroupsHtml;
  if (partiesPerGroup === 1 && raidGroups.length > 0) {
    // 4인 레이드: 모든 파티를 한 그리드에
    autoGroupsHtml = renderFlatPartyGrid({ raid, raidGroups, isManual: false });
  } else {
    autoGroupsHtml = raidGroups.map((group, gi) =>
      renderGroupBlock({
        raid, gi, group,
  ...
  // 수동 공격대 렌더링 (수동은 사용자가 직접 만든 거라 항상 별도 헤더로 분리)
 ...
 */
/**
 * 4인 레이드 전용: 모든 공격대의 파티를 하나의 grid에 평탄화하여 렌더링.
 * 8인 레이드는 renderGroupBlock으로 공격대 단위 분리 유지.
 */
function renderFlatPartyGrid({ raid, raidGroups, isManual }) {
  const partiesHtml = raidGroups.map((group, gi) => {
    return group.parties.map((party, pi) => {
      return renderPartyCard({ raid, gi, pi, party, isManual, partiesPerGroup: 1 });
    }).join('');
  }).join('');
...
/**
 * 파티 카드 하나를 렌더링 (renderGroupBlock과 renderFlatPartyGrid에서 공통 사용)
 */
function renderPartyCard({ raid, gi, pi, party, isManual, manualGroupId, partiesPerGroup, group }) {
  ...
  // 파티 이름
  const partyNumberFor4 = gi + 1;
  const partyNumberFor8 = pi + 1;
  const defaultPartyName = partiesPerGroup > 1
    ? `${raid.name} ${gi + 1}공격대 ${partyNumberFor8}파티`
    : `${raid.name} ${partyNumberFor4}파티`;
  const looksAutoGenerated = (label) => {
    if (!label) return true;
    if (/^(파티|공격대)\s*\d+$/.test(label)) return true;
    if (/^\d+(파티|공격대)$/.test(label)) return true;
    if (/.+\s+\d+파티$/.test(label)) return true;
    if (/.+\s+\d+공격대\s+\d+파티$/.test(label)) return true;
    return false;
  };
  let partyName;
  if (isManual) {
    const stored = group && group.partyLabels && group.partyLabels[pi];
    partyName = (stored && !looksAutoGenerated(stored)) ? stored : defaultPartyName;
  } else {
    const stored = getAutoPartyLabel(raid.id, gi, pi, null);
    partyName = (stored && !looksAutoGenerated(stored)) ? stored : defaultPartyName;
  }
...
function renderGroupBlock({ raid, gi, group, isManual, manualGroupId, partiesPerGroup, label }) {
  const partiesHtml = group.parties.map((party, pi) =>
    renderPartyCard({ raid, gi, pi, party, isManual, manualGroupId, partiesPerGroup, group })
  ).join('');

  // 공격대 이름 (편집 가능)
  // 8인 레이드만 공격대 헤더 표시. 수동은 항상 표시.
  const looksAutoGroupLabel = (l) => {
    if (!l) return true;
    if (/^(공격대|파티)\s*\d+$/.test(l)) return true;
    if (/^\d+(공격대|파티)$/.test(l)) return true;
    if (/.+\s+\d+공격대$/.test(l)) return true;
    return false;
  };
  const defaultGroupLabel = partiesPerGroup > 1
    ? `${raid.name} ${gi + 1}공격대`
    : `${raid.name}`;
  let displayLabel;
  if (isManual) {
    displayLabel = group.label;
  } else if (partiesPerGroup > 1) {
    const stored = getAutoGroupLabel(raid.id, gi, null);
    displayLabel = (stored && !looksAutoGroupLabel(stored)) ? stored : defaultGroupLabel;
  } else {
    displayLabel = null; // 4인 레이드는 공격대 헤더 없음
  }
  ...
  const labelEl = showHeader
    ? `<input type="text" class="editable-label"
            ...
            data-default="${escapeHtml(defaultGroupLabel)}"
            ${groupLabelAttr}
            title="클릭해서 이름 수정">`
    : '';
...
function renderCharChip(char) {
  ...
  const hasPower = char.combatPower > 0;
  const powerDisplay = hasPower ? char.combatPower.toLocaleString() : '?';
  const noPowerWarn = !hasPower ? '<span class="chip-no-power" title="전투력 정보 없음">?</span>' : '';
  ...
  return `
    <div class="char-chip ${supportClass} ${isDup ? 'is-duplicate' : ''}"
      ...
      <div class="chip-class">${escapeHtml(char.className)}</div>
      <div class="chip-name">${escapeHtml(char.name)}${badge}</div>
      <div class="chip-stats">
