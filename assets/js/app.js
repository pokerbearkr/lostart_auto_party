...
function renderGroupBlock({ raid, gi, group, isManual, manualGroupId, partiesPerGroup, label }) {
  const partiesHtml = group.parties.map((party, pi) => {
    ...
    // 파티 기본 이름:
    //  - 4인 레이드: 공격대마다 파티 1개씩 → "지평의 성당 1단계 1파티", "...2파티", ...
    //  - 8인 레이드: 공격대 안에 파티 2개 → "종막 1공격대 1파티", "종막 1공격대 2파티"
    ...
    // 사용자가 명시적으로 수정한 라벨만 인정.
    // 수동 공격대는 partyLabels에, 자동은 autoLabels에 사용자가 직접 입력한 것만 저장됨.
    // 하지만 이전 버전에서 자동 생성된 라벨이 잘못 저장된 경우가 있어,
    // 기본값과 동일하거나 패턴이 자동 라벨인 경우는 무시하고 새 기본값 사용.
    const looksAutoGenerated = (label) => {
      if (!label) return true;
      // 옛 자동 패턴들 (모두 폐기)
      if (/^(파티|공격대)\s*\d+$/.test(label)) return true;
      if (/^\d+(파티|공격대)$/.test(label)) return true;
      // 어떤 레이드의 자동 패턴 (예: "지평의 성당 1단계 1파티")
      // → 이런 형식도 자동 생성된 것으로 판단해서 새 기본값으로 대체
    ...
    let partyName;
    if (isManual) {
      const stored = group.partyLabels && group.partyLabels[pi];
