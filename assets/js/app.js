...
function renderGroupBlock({ raid, gi, group, isManual, manualGroupId, partiesPerGroup, label }) {
  const partiesHtml = group.parties.map((party, pi) => {
    ...
    // 사용자가 커스텀 이름을 저장한 경우만 그걸 쓰고,
    // 이전 버전의 자동 라벨(예: "파티 1") 같은 옛날 기본값은 무시
    const isLegacyAutoLabel = (label) => {
      if (!label) return false;
      // 옛 형식: "파티 N" 단독, "공격대 N" 단독 등은 자동 라벨이었으므로 폐기
      return /^(파티|공격대)\s+\d+$/.test(label);
    };
    let partyName;
    if (isManual) {
      const stored = group.partyLabels && group.partyLabels[pi];
      partyName = (stored && !isLegacyAutoLabel(stored)) ? stored : defaultPartyName;
    } else {
  ...
  const isLegacyGroupLabel = (l) => l && /^(공격대|파티)\s+\d+$/.test(l);
  ...
  if (isManual) {
    ...
    displayLabel = (stored && !isLegacyGroupLabel(stored)) ? stored : defaultGroupLabel;
  } else {
